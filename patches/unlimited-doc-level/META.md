...
```js
if (obj.data.level != 3) { /* 可选为上级 */ }
else { layer.msg("第三级文档不能作为上级文档") }
```

本补丁去掉该判断，任意节点都可作上级（修改页仍禁止选自己）。
...
后端放开后，前端仍会弹出「第三级文档不能作为上级文档」，必须两处一起改。
...
| 2026-09-09 | 整理为「与官方同路径可直接覆盖」的完整文件包 |
