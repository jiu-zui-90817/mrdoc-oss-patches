...
def get_level_docs(request):
    """获取文集文档层级列表（Token API，客户端用）。支持无限层级。"""
    token = request.GET.get('token', '')
    try:
        token = UserToken.objects.get(token=token)
        pid = request.GET.get('pid', '')
        ...
        # 一次取出所有已发布文档，递归构建树（与 Web 端 get_pro_toc 同思路）
        all_docs = list(
            Doc.objects.filter(top_doc=pid, status=1)
            .values('id', 'name', 'editor_mode', 'parent_doc')
            .order_by('sort')
        )
        children_map = {}
        for d in all_docs:
        ...
        def build_node(doc):
            ...
            for child in children_map.get(doc['id'], []):
                item['sub'].append(build_node(child))
            return item

        doc_list = [build_node(d) for d in children_map.get(0, [])]
        doc_cnt = len(all_docs)

        # 不需要分页
        if is_page is False or str(is_page).lower() in ('false', '0'):
            return JsonResponse({'status': True, 'data': doc_list, 'total': doc_cnt})

        # 分页仍按「一级文档」分页（与官方行为一致）
