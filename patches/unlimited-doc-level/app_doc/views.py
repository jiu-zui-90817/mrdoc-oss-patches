...
def get_pro_toc(pro_id):
    """递归构建文集文档目录树，支持无限层级"""
    # 一次取出该文集所有已发布文档，避免按层反复查库
    all_docs = list(
        Doc.objects.filter(top_doc=pro_id, status=1)
        .values('id', 'name', 'parent_doc', 'open_children', 'editor_mode')
        .order_by('sort')
    )
    ...
    children_map = {}
    for d in all_docs:
        children_map.setdefault(d['parent_doc'], []).append(d)

    def build_node(doc):
        item = {
            'id': doc['id'],
            'name': doc['name'],
            'open_children': doc.get('open_children', False),
            'editor_mode': doc['editor_mode'],
            'editor_icon': EDITOR_MODE_ICON_MAP.get(doc['editor_mode'], ''),
        }
        kids = children_map.get(doc['id'], [])
        if kids:
            item['sub'] = [build_node(c) for c in kids]
        return item

    top_docs = children_map.get(0, [])
    doc_list = [build_node(d) for d in top_docs]
    n = len(all_docs)
    return (doc_list, n)
...
def manage_project_doc_sort(request,pro_id):
    if request.method == 'GET':
        ...
        if (pro.create_user == request.user) or pro_colla.count() > 0:
            ...
            def build_sort_node(doc, level=1):
                node = {
                    'id': doc['id'],
                    'field': doc['name'],
                    'title': doc['name'],
                ...
                kids = children_map.get(doc['id'], [])
                if kids:
                    node['children'] = [build_sort_node(c, level + 1) for c in kids]
                return node

            doc_list = [build_sort_node(d, 1) for d in children_map.get(0, [])]
        ...
        if (pro.create_user == request.user) or pro_colla.count() > 0:
            # 文档排序（递归，支持无限层级）
            def apply_sort(nodes, parent_id=0):
                n = 10
                for node in nodes:
                    Doc.objects.filter(id=node['id']).update(sort=n, parent_doc=parent_id)
                    n += 10
                    if 'children' in node and node['children']:
                        apply_sort(node['children'], parent_id=node['id'])

            apply_sort(sort_data, parent_id=0)
            return JsonResponse({'status': True, 'data': 'ok'})
        else:
...
# 获取指定文集的所有文档（支持任意层级，供选择上级文档使用）
@require_http_methods(["POST"])
@logger.catch()
def get_pro_doc(request):
    """获取文集文档列表，供选择上级文档使用，支持任意层级"""
    pro_id = request.POST.get('pro_id', '')
    if pro_id == '':
        return JsonResponse({'status': False, 'data': _('参数错误')})
    ...
    docs = list(
        Doc.objects.filter(top_doc=int(pro_id), status=1)
        .values('id', 'name', 'parent_doc')
        .order_by('sort', 'id')
    )
    id_to_name = {d['id']: d['name'] for d in docs}
    id_to_parent = {d['id']: d['parent_doc'] for d in docs}

    def build_path(doc_id):
        """拼出 父 --> 祖父 --> ... 的路径前缀，方便选择时辨认层级"""
        parts = []
        pid = id_to_parent.get(doc_id, 0)
        # 最多往上走 50 层，防止脏数据成环
        for _ in range(50):
            if not pid:
                break
            parts.append(id_to_name.get(pid, str(pid)))
            pid = id_to_parent.get(pid, 0)
        if not parts:
            return ''
    ...
    return JsonResponse({'status': True, 'data': item_list})
...
def get_pro_doc_tree(request):
    ...
    if pro_id:
        ...
        all_docs = list(
            Doc.objects.filter(top_doc=pro_id, status=1)
            .values('id', 'name', 'parent_doc', 'modify_time')
            .order_by('sort')
        )
        children_map = {}
        for d in all_docs:
        ...
        def build_tree_node(doc, level=1):
            node = {
                'id': doc['id'],
                'field': doc['name'],
                'title': doc['name'],
            ...
            kids = children_map.get(doc['id'], [])
            if kids:
                node['children'] = [build_tree_node(c, level + 1) for c in kids]
            return node

        doc_list = [build_tree_node(d, 1) for d in children_map.get(0, [])]
